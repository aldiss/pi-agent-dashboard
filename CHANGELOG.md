...
### Fixed
- **PWA cold-load no longer shows blank-white screen for 20+s — app-shell skeleton paints instantly + session-list populates within ~2s + Service Worker caches the shell for warmed reloads.** Three-deliverable composite for JadeIce mobile-ux-audit Cluster A n=6 P0 root-cause (Service Worker explicitly disabled caching per pre-fix `public/sw.js`; client-side fetch of `/api/sessions` was missing so the session list waited for a WebSocket mutation event that never arrived during the cold-load window). (a) `packages/client/src/index.html` now ships an inline app-shell skeleton (PI logo + "Loading PI Dashboard…" + spinner + dark theme; ~50 LOC HTML+CSS; React render-pass replaces in a single atomic DOM mutation per React 19 `createRoot` canonical). (b) `public/sw.js` rewritten v2→v3: vanilla Service Worker (no Workbox dependency) with `pi-dashboard-precache-v3` for app-shell assets (cache-first navigation + static) + `pi-dashboard-api-v3` for runtime `/api/*` (network-first with cache fallback, 3s timeout) + bypass-cache for `/ws/*` + Web Push handler + notification click handler PRESERVED VERBATIM from v2 (zero-regression). New post-build script `packages/client/scripts/inject-sw-precache-manifest.mjs` (zero-dep Node, sister-shape to `precompress.mjs`) enumerates Vite-emitted hashed assets from `dist/index.html` and substitutes the `__PRECACHE_MANIFEST__` placeholder. (c) New `packages/client/src/hooks/useSessionsBootstrap.ts` HTTP-fetches `/api/sessions` on mount with race-safe `current.size > 0` size-guard against WebSocket `sessions_snapshot` REPLACE-semantics. Playwright pre-ship verification against the live Tailscale deploy confirmed: FCP=92ms throttled / 52ms warmed / 75ms aggressive-reinstall (5–10× margin under ≤500ms target); `caches.keys()` returns 2 stores; 24 assets served `fromServiceWorker=true` on warmed reload; `/api/sessions` 45–107ms baseline (well under ≤200ms); session-list populates 1237–1365ms (28–33× faster than the never-populated-in-40s baseline). **Operator action required (one-time, per device): close the PI Dashboard PWA on your iPhone (swipe up, swipe the PI tile away — not just background it), then reopen from the home screen.** The new Service Worker activates on relaunch. If the dashboard still looks stuck after close+reopen (rare iOS standalone-PWA edge), do the aggressive reinstall: long-press the PWA icon → Remove App → Delete App → open the dashboard URL in Safari → Share → Add to Home Screen. Same drill on iPad if used there. Honest notes: runtime API cache has no implemented TTL at v0 (progressive-enhancement shape — show last-known session list on flaky network; banked to v0.5+ `api-cache-ttl-hardening` follow-up); `CACHE_VERSION` bump is manual at v0 (banked to v0.5+ `sw-cache-version-automation` follow-up with content-hash-of-manifest correction, NOT git-rev-parse which would over-invalidate); `networkFirstWithCache` does not abort the underlying fetch on timeout (banked to v0.5+ `sw-network-first-fetch-abort-hardening` follow-up). Real-iPhone PWA operator-empirical-cycle-pass is required per AGENTS.md v2.0 Mega-Cluster M tier-(b) UI-empirical-cycle gate (Playwright/WebKit pre-ship is strong evidence but cannot reproduce iOS standalone-PWA SW activation semantics). (change: `dashboard-pwa-cold-load-fix`; cell: `dashboard-pwa-cold-load-fix/v1`; sister-coupling: HappyQuartz `voice-input-hardening/v1` W5-amendments §1+§3+§4 PWA SW cache discipline + AGENTS.md Mega-Cluster M v2.0 tier-(a) extension sub-clause (iv) PWA-or-SW-cache-bound canonical territory)
