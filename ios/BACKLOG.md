...
| F8 | External sessions are listed but cannot be opened | SHIPPED `b00c894` | F3 shipped the `external` TIER and the list SOURCE, and was correctly marked shipped for that. Openability was never recorded as an item, so it reached the operator as a surprise on 2026-09-04 rather than as a known gap: he could see Codex / Claude Code sessions in the list and could not open them the way the PWA does. Cause was structural, not a regression — `SessionListView` rendered the external card with NO `NavigationLink`, and the app never called `/capture` or `/transcript` (verified: 0 call sites, against a control proving the list endpoint IS called). Server side already existed and needed no change; the PWA already had `ExternalSessionsPanel`. Now opens a read-only transcript reusing the existing chat rows, with defensive decoding (an unknown entry kind does not fail the whole decode) and a percent-encoded path, since external ids contain a colon. Read-only by construction: no compose, send or abort path exists in the view. LESSON: a shipped row is not a shipped feature, and a backlog item titled by its data source will read as done while its user-facing half is missing. |
...
| ID | Title | Status | Detail |
|---|---|---|---|
| B10 | Socket accepted then dropped ~250ms later, repeatedly | OPEN — **client EXONERATED by evidence**; trigger still unattributed | Measured on the gateway log: accepted, dead 186-337ms later, retrying ~2.3s apart, while a browser on the same server stayed connected and the server logged no reason. Ruled out earlier with controls: undecodable frames, oversize snapshot (1.2MB vs a 4MB cap), auth rejection (no 401, and that log demonstrably records 401s). NOW ALSO RULED OUT: the client itself. The `session-lifecycle` harness drives the real store through connect, snapshot, subscribe, live event, session update, foreground return, chat switch and a genuinely silent idle window, asserting on the SERVER's trace. Result: `ACCEPTED=1 max-live=1 ws-pings=3 idle-app-frames=0` — one socket throughout, kept alive by protocol pings with zero application traffic, which is the real-world condition. Its must-fail control forces a real second socket and IS rejected (`ACCEPTED=2 max-live=2`), so the assertion can fail. Remaining suspects are therefore the server, the Cloudflare tunnel, or the mobile network — not the app. To close it: the browser gateway's close handler is `ws.on("close", () => {` with NO parameters (`browser-gateway.ts:1056` on origin/main) — the `ws` library passes `(code, reason)` and the site discards them by never naming them. The consumer already exists and is tested: `bridge-disconnect-classifier.ts` declares `closeCode?: number` (:38) with `classifyBridgeDisconnect` (:63). So the fix is naming two arguments and feeding an existing classifier, not building one. CAVEAT, and it bounds the value: an RFC 6455 code can be absent entirely on an abrupt transport drop, so this is NOT decisive between server, tunnel and network — it distinguishes an orderly close from an abrupt drop, which splits the space but does not settle it. Server-side, Joan's domain; diagnosis banked, not built. CLIENT HALF, FOUND 2026-09-04 AND NOW SHIPPED (`f7db6a2`): the client had the mirror-image defect and nobody had checked it. `receiveLoop`'s catch kept only `error.localizedDescription` — a string that reads identically whether a peer closed deliberately or the transport vanished — and never read `socket.closeCode` / `.closeReason`, which the task is still holding at that point. There is no `URLSessionWebSocketDelegate` either (verified with a control: 0 delegate hits against 20 `URLSession` hits). So with BOTH ends mute, every B10 investigation has argued from an absence that is uninformative by construction, which is a candidate explanation for why it has outlived several tenures. `SocketCloseClassifier` now classifies orderly-vs-abrupt and folds it into the failure state, mirroring the server's existing `bridge-disconnect-classifier` vocabulary. The same CAVEAT bounds the client half exactly as it bounds the server half: it splits orderly from abrupt, and does NOT attribute server vs tunnel vs network. This does not fix B10; it makes the next occurrence produce evidence instead of nothing. |
| B11 | `revalidate()` does not fail pending local sends, where `scheduleReconnect()` does | OPEN — asymmetry observed, impact unproven | The foreground-return path sets `phase = .reconnecting` and calls `startStream` without calling `failPendingOperationsOnDisconnect()`, which the other reconnect path does call. If the subsequent reconnect never completes, an optimistic entry has no fast-fail from this path; the 90s `queueAckDeadline` task is still armed and independent, so the entry is not orphaned outright. Found while diagnosing an operator-reported stuck "1 queued" on 2026-09-04. NOT shown to be implicated in that report — the stuck entry was almost certainly `confirmed` (the bridge had acked it; the message was verified delivered by reading the target session's pane), and `confirmed` is deliberately never failed by either path. Recorded rather than fixed, because a speculative fix here would invent a loss that did not occur. |
...
| V1 | Physical microphone acceptance | UNRUN — requires the operator's device |
| V2 | Physical OAuth + real Keychain migration over an installed build | UNRUN — green results to date are against a memory stub, not the Security framework |
| V4 | `ForegroundProbeTests` timing test flakes intermittently | OPEN — observed twice independently on 2026-09-04, not diagnosed. Once during a restore-residue check (1 failure, then unreproduced across 12 consecutive runs, name not captured at the time) and once by the external-viewer build, which named it: `ForegroundProbeTests`, a timing test, flaked on two intermediate runs and passed unchanged on rerun. Two independent observers, same suite. This matters beyond noise — the suite gates every push, so a timing-dependent test can redden CI at random and train the crew to re-run instead of read. Not yet reproduced deliberately, so the mechanism is unknown. |
