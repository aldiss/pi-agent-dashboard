...
## Legend
- ✅ **verified** — parity proven by a test that is **green today** via
  `cd ios/PiDashboardKit && swift test` (no simulator). 108 tests, 0 failures.
- 🟡 **partial** — test **authored + type-checks** but the **run is pending**
  SwiftPilot's app-branch integration (XCUITest needs the simulator target), or
  the logic is ✅ while the UI-layer e2e is pending, or a positive path needs a
  small app hook.
...
(authored in `qa-e2e/PiDashboardUITests/`, runs post-integration).
...
| Search filter (name → firstMessage → cwd basename) | `list-search` → `filterByQuery` | `swift:GroupingPropertyTests` (`testFilterByQueryIsSubsetAndIdempotent`); `e2e:F7` (`testF7_SearchNarrowsCards`) | ✅ logic / 🟡 e2e |
| Folders on/off (flatten directory subgroups) | `toggle-folders` → `groupTierByFolder folders:false` | `swift:DirectoryGroupingTests` (`testGroupTierByFolder_offFlattens`); `e2e:F7` (`testF7_FoldersToggleFlattensDirectoryGroups`) | ✅ logic / 🟡 e2e |
| Hide-stale (threshold hrs) drops stale-active | `toggle-hide-stale` → `filterStale` | `swift:GroupingTests`, `swift:GroupingPropertyTests` (`testFilterStaleMonotoneInThreshold`); `e2e:F7` (`testF7_HideStaleToggleFlipsState`) | ✅ logic / 🟡 e2e |
...
| PWA behavior | Native behavior | Test id | Status |
|---|---|---|---|
| Open session → chat surface + composer mount; app sends `session_view` | tap `session-card-<id>` → `chat-scroll` + `mobile-composer`; `openSession` → `session_view` | `e2e:F3` (`testF3_OpenSessionShowsChatAndComposer`); wire msg `swift:ProtocolRoundTripTests` (`testEncodeEverySingleSessionIdMessage`) | 🟡 e2e / ✅ wire |
...
| `hasNewline ? true` — newline always multiline | `ComposerLayout.isMultiline` newline clause | `swift:ComposerModelPropertyTests` (`testNewlineForcesMultilineUniversally`); `e2e:F4` (`testF4_NewlineForcesMultilineAndNeverSends`) | ✅ logic / 🟡 e2e |
...
| Send enabled iff trimmed-nonempty OR ≥1 image (not gated on streaming) | `ComposerLayout.canSend` + `mobile-composer-send` enabled state | `swift:ComposerModelPropertyTests` (`testCanSendTruthTable`); `e2e:F5` (`testF5_SendButtonGating`) | ✅ logic / 🟡 e2e |
...
| PWA behavior | Native behavior | Test id | Status |
|---|---|---|---|
| Connection banner on >3s disconnect | `connection-banner` (shown while `.reconnecting`/`.failed`) | `e2e:F6` (`testF6_NoBannerWhileConnected` ✅ runnable; `testF6_BannerAppearsWhenReconnecting` pending app hook) | 🟡 |
...
| Layer | Verified now (✅) | Authored, pending integration (🟡) | Deferred (⛔) |
|---|---|---|---|
...
| UI flows F1–F7 | F6-negative (logic green) | F1–F5, F7 (XCUITest authored + type-checks), F6-positive (needs `-uitest-reconnecting` hook) | haptics, server-restart states |

**Bottom line:** every logic + contract surface of the MVP is ✅ green today via
`swift test` (grounded in REAL captured payloads, not impressions). Every UI flow
F1–F7 has an authored, type-checked XCUITest that runs once SwiftPilot integrates
the app branch. Deferred rows are MVP-scope exclusions per DESIGN §7.
