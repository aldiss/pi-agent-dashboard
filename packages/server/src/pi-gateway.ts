    ...
    start(port: number) {
      ...
      wss.on("connection", (ws) => {
        ...
        ws.on("close", (code?: number) => {
          ...
          // W1b cross-wire cleanup — OUTSIDE the CAS guard (must-fix): a displaced
          // socket is add()ed at a register site then terminate()d by setConnection,
          // so its close fails the CAS guard; deleting inside the guard would leak the
          // Set entry every displacement (~100×/reload-storm = the dangling FD-set the
          // G5 leak-fix targets). Unconditional per-ws cleanup, next to aliveMisses.
