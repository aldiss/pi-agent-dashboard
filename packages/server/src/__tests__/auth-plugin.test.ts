...
describe("validateWsUpgrade", () => {
  it("should allow localhost without cookie", () => {
    expect(validateWsUpgrade(undefined, "127.0.0.1", SECRET).allowed).toBe(true);
    expect(validateWsUpgrade(undefined, "::1", SECRET).allowed).toBe(true);
    expect(validateWsUpgrade(undefined, "::ffff:127.0.0.1", SECRET).allowed).toBe(true);
  });

  it("should reject external request without cookie", () => {
    expect(validateWsUpgrade(undefined, "1.2.3.4", SECRET).allowed).toBe(false);
  });

  it("should reject external request with invalid cookie", () => {
    expect(validateWsUpgrade(`${COOKIE_NAME}=invalidtoken`, "1.2.3.4", SECRET).allowed).toBe(false);
  });
  ...
  it("should allow external request with valid cookie", () => {
    ...
    const decision = validateWsUpgrade(`${COOKIE_NAME}=${token}`, "1.2.3.4", SECRET);
    expect(decision.allowed).toBe(true);
    // Build 0: the decision now carries the decoded principal, not just a bool.
    expect(decision.principal?.sub).toBe("user@example.com");
  });
  ...
  it("should reject external request with wrong secret", () => {
    ...
    expect(validateWsUpgrade(`${COOKIE_NAME}=${token}`, "1.2.3.4", SECRET).allowed).toBe(false);
  });
});
