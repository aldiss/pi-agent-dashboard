...
import { Sheet, Pressable } from "../motion/index.js";
 ...
 * desktop-only (gated `!isMobile` in the StatusBar).
 *
 * Built on the <Sheet> motion primitive: GENTLE spring-enter from the bottom +
 * a REAL drag-to-dismiss grab handle (throw it down to close; a weak flick
 * springs back) + a scrim that dims with the drag. The rows are <Pressable>, so
 * each scales under the finger with a selection haptic. Tokenized, so it wears
 * the editorial warm skin or the legacy skin automatically.
 *
 * Dismiss contract preserved: picking a model closes the sheet immediately
 * (direct onClose); thinking + bell taps keep it open; scrim / Esc / throw run
 * the animated exit then unmount.
 */
export function ModelReasoningSheet({
  ...
  return (
    <Sheet
      ...
      scrimTestid="model-sheet-scrim"
      className="border-t border-[var(--border-secondary)] bg-[var(--bg-secondary)] shadow-2xl"
    >
      {/* Pinned title (the grab handle is owned by <Sheet> above this). */}
      ...
      <div className="overflow-y-auto px-5 pt-3 flex-1 min-h-0">
