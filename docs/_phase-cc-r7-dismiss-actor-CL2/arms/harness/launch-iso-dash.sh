...
# Temp HOME, loopback-forced guard, scratch ports 8155/8156, reclaim DISABLED,
...
WS="/tmp/build1-ccr7-cand-8155"
export HOME="$WS/state"
export PI_DASHBOARD_NO_RECLAIM=1
export PI_DASHBOARD_ALLOW_MULTIPLE=1
export PI_DASHBOARD_PORT=8155
export PI_DASHBOARD_PI_PORT=8156
export PI_DASHBOARD_URL="ws://localhost:8156"
export LOOPBACK_GUARD_LOG="$WS/evidence/loopback-guard.log"
export NODE_OPTIONS="--require $WS/harness/loopback-listen-guard.cjs"
cd "$WT" || exit 1
exec node "$WT/packages/server/bin/pi-dashboard.mjs" start --port 8155 --pi-port 8156 \
