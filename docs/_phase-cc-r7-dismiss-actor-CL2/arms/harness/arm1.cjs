// CC-r7 arm1 — opt1 bijection on the staged CANDIDATE build (7fc75b5). Drive the
// natural healthy select, REAL DOM: click "Run one more validation pass first"
// (index 1), assert returned == original[1] (bijection), receipt answered @
// dashboard source, question visible, 3 distinct plain labels, zero :9999/:8000.
...
(async () => {
  const tag = "arm1-opt1-" + Date.now();
  const sid = await H.spawnSession(tag);
  const rec = { arm: "opt1-bijection", staged_head: STAGED, option_index: I, expected_original: OPTS[I], session: sid };
