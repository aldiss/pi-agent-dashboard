...
(async () => {
  ...
  if (sidA) {
    ...
    // dl-13559 DOM ASSERTION: after the bus timeout, the mounted card must read a
    // truthful non-answer ("No response") and must NEVER show "Answered in
    // terminal" (which falsely claims a TUI answer). Read the visible body text.
    const bodyText = await pg.innerText("body").catch(() => "");
    result.rendered.post_timeout_body_excerpt = bodyText.replace(/\s+/g, " ").slice(0, 600);
    result.rendered.assert_dom_shows_no_response = /No response/.test(bodyText);
    result.rendered.assert_dom_no_answered_in_terminal = !/Answered in terminal/.test(bodyText);
  ...
  result.A1_LIVE_CONTRAST_PROVEN =
    !!(result.rendered.dialog_mounted && result.rendered.assert_delivered_true && result.rendered.assert_rendered_true &&
       ...
       // dl-13559: the DOM must show the truthful non-answer, never "Answered in terminal".
       result.rendered.assert_dom_shows_no_response && result.rendered.assert_dom_no_answered_in_terminal &&
